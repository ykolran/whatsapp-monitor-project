...
class ConversationAdapter(
    ...
    // ── Swipe-to-archive ItemTouchHelper ──────────────────────────────
    fun attachSwipeHelper(recyclerView: RecyclerView) {
        val helper = ItemTouchHelper(SwipeCallback(recyclerView.context))
    ...
    private inner class SwipeCallback(private val context: Context)
        ...
        private val background = ColorDrawable(ContextCompat.getColor(context, R.color.swipe_archive))
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
            ...
            onSwipe(items[position], position)
        }
